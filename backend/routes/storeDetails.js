...
router.get('/', async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT * FROM store_details LIMIT 1');
    if (rows.length === 0) {
      ...
      await pool.execute(`
        ...
        VALUES ('Bookstore', '', '', '', '', '', '', '', '')
      `);
      const [newRows] = await pool.execute('SELECT * FROM store_details LIMIT 1');
  ...
  try {
    ...
    const [result] = await pool.execute(`
      UPDATE store_details SET 
        name=?, contact=?, website=?, address=?, fax=?, 
        email=?, tax_number=?, receipt_footer=?, logo=?
    ...
    const [updatedRows] = await pool.execute('SELECT * FROM store_details LIMIT 1');
