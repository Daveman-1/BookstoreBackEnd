...
router.get('/daily-sales', authenticateToken, authorizeRoles('admin', 'manager'), async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT * FROM daily_sales_summary_view ORDER BY sale_date DESC LIMIT 30');
...
router.get('/top-selling', authenticateToken, authorizeRoles('admin', 'manager'), async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT * FROM top_selling_items ORDER BY total_sold DESC LIMIT 20');
...
router.get('/low-stock', authenticateToken, authorizeRoles('admin', 'manager'), async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT * FROM low_stock_items');
